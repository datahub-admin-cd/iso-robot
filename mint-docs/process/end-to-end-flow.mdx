...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>[11 Chatbot (SSE RAG)](/flow/11-chatbot)</td>
      <td>A question in plain English</td>
      <td>Streamed, cited answer grounded in the org's own records</td>
      <td>SSE stream (+ one-time reindex job)</td>
    </tr>
  </tbody>
</table>
...
<Steps>
  ...
  <Step title="Chatbot — build knowledge index">
    `POST /chatbot/reindex` (once) → poll job → confirm `org_indexed_chunks > 0` via `GET /chatbot/status`.
  </Step>
  <Step title="Chatbot — ask questions">
    `POST /chatbot/query` with `{ "question": "..." }` → read the SSE stream (`retrieval` → `message` → `done`).
  </Step>
</Steps>
